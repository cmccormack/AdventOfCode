...
module.exports = (str, start=0) => {
  ...
  // Determine the top sleeper's ID
  const topSleeperId = Object.keys(guards).map(guard => ({
      id: guard,
      slept: Object.values(guards[guard]).reduce((a,v) => a += v.length, 0),
  })).reduce((a,v) => a.slept > v.slept ? a : v).id

  // Find the most slept minute for top sleeper
  const bin = {}

  for (set of Object.values(guards[topSleeperId])) {
    for (min of set) {
      if (!bin[min]) {
        bin[min] = 0
      }
  ...
  return Object.entries(bin).sort((a,b) => b[1] - a[1])[0][0] * topSleeperId
}
